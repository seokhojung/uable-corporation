...
export function ImpactStats({ detailContent }: ImpactStatsProps) {
  ...
  return (
    <section className={`py-16 ${themeClasses.statsBackground}`}>
      <div className="max-w-6xl mx-auto px-4">
        ...
        >
          <h2 className={`text-3xl font-bold ${themeClasses.statsTitle} mb-6`}>
            프로젝트 임팩트
          </h2>
          <div className={`${themeClasses.overviewCard} rounded-2xl p-8 ${themeClasses.overviewCardBorder} max-w-4xl mx-auto`}>
            <p className={`${themeClasses.overviewText} leading-relaxed text-lg`}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {stats.map((stat, index) => (
            <motion.div
              ...
              className={`${themeClasses.statCard} backdrop-blur-sm rounded-2xl p-8 ${themeClasses.statCardBorder} transition-all duration-300`}
            >
              {/* 큰 숫자 */}
              <div className="relative mb-6">
                ...
                >
                  <div className={`text-6xl lg:text-7xl font-semibold ${themeClasses.statNumber} mb-2 tracking-tight`}>
                    {stat.value}
                  </div>
                  <div className={`w-12 h-px ${themeClasses.statDivider} mx-auto opacity-65`}></div>
                </motion.div>
              </div>

              {/* 제목과 설명 */}
              <div className="text-center space-y-3">
                <h3 className={`text-lg font-semibold ${themeClasses.statTitle}`}>
                  {stat.title}
                </h3>
                <p className={`text-sm ${themeClasses.statDescription} leading-relaxed`}>
                  {stat.description}
                </p>
              </div>
            </motion.div>
        ...
        <motion.div
          ...
          className={`mt-16 ${themeClasses.impactCard} rounded-2xl p-8 text-center ${themeClasses.impactCardBorder}`}
        >
          <h3 className={`text-xl font-semibold ${themeClasses.statTitle} mb-4`}>
            비즈니스 성과
          </h3>
          <p className={`${themeClasses.impactText} text-base leading-relaxed max-w-4xl mx-auto`}>
